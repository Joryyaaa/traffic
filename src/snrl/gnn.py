...
def adjacency_mask(adjacency: np.ndarray) -> torch.Tensor:
    ...
    Returns a (n_nodes, n_nodes) bool tensor: True where attention is allowed.
    """
    a = np.asarray(adjacency, dtype=np.float32)
    a_hat = a + np.eye(a.shape[0], dtype=np.float32)
...
class GATLayer(nn.Module):
    ...
    def forward(self, h: torch.Tensor, adj_mask: torch.Tensor) -> torch.Tensor:
        ...
        # adj_mask: (n_nodes, n_nodes), 1 where attention allowed
        B, N, _ = h.shape
        Wh = self.W(h).view(B, N, self.n_heads, self.head_dim)  # (B, N, H, D)

        # Compute attention logits: e_ij = a_l^T Wh_i + a_r^T Wh_j
        # (B, N, H) each
        e_l = (Wh * self.a_l).sum(dim=-1)  # (B, N, H)
        e_r = (Wh * self.a_r).sum(dim=-1)  # (B, N, H)
        # (B, N, N, H): broadcast i over rows, j over cols
        e = e_l.unsqueeze(2) + e_r.unsqueeze(1)  # (B, N_i, N_j, H)
        e = self.leaky_relu(e)

        # Mask: set non-neighbours to -inf before softmax
        mask = adj_mask.unsqueeze(0).unsqueeze(-1)  # (1, N, N, 1)
        e = e.masked_fill(mask == 0, float("-inf"))
        alpha = torch.softmax(e, dim=2)  # softmax over j (source nodes)
        alpha = alpha.nan_to_num(0.0)  # isolated nodes: all -inf → nan → 0
